...
from pyquantumkit import CircuitIO, new_mixed_circuit, get_n_qubits, get_n_cbits, get_qubit_list, copy_mixed_circuit,\
      get_framework_from_object, append_mixed_circuit, apply_measure, run_and_get_counts, parallel_mixed_circuits
...
# Implement the relation checking for quantum circuits,
...
def run_equivalence_check(qvm, TargetProc1, TargetProc2,
    ...
    """
    Run equivalence checking for a pair of quantum circuits.

        qvm : run on which quantum machine
        TargetProc1 : the first quantum circuit
        TargetProc2 : the second quantum circuit
    ...
    if (Nqs != get_n_qubits(TargetProc2)):
        return False
    if Nqs == 0:        #if the target procs are both with size 0
        return True
    ...
    tp1 = copy_mixed_circuit(TargetProc1)
    tp2 = copy_mixed_circuit(TargetProc2)

    for i in range(0, NPoints):
        STprocA = new_mixed_circuit(framework, 2 * Nqs, 2 * Ncs1)
        STprocB = new_mixed_circuit(framework, 2 * Nqs, 2 * Ncs2)
        STprocAB = new_mixed_circuit(framework, 2 * Nqs, Ncs1 + Ncs2)
...
def run_identity_check(qvm, TargetProc, NPoints : int = Default_Identity_NPoints) -> bool:
    """
    Run identity checking for a quantum circuit.

        qvm : run on which quantum machine
        TargetProc : target quantum circuit
...
def run_keep_purity_check(qvm, TargetProc,
    ...
    """
    Run keep-purity checking for a quantum circuit.
    ...
    qlist2 = [x + Nqs for x in qlist1]
    tp = copy_mixed_circuit(TargetProc)

    for i in range(0, NPoints):
        STproc = new_mixed_circuit(framework, 2 * Nqs, 2 * Ncs)
        randompaulis = [randint(0, 5) for _ in qlist1]

        create_pauli_eigenstate(STproc, randompaulis, qlist1)
        create_pauli_eigenstate(STproc, randompaulis, qlist2)
        append_mixed_circuit(STproc, parallel_mixed_circuits(tp, tp))
...
def run_unitarity_check(qvm, TargetProc,
    ...
    """
    Run unitarity checking for a quantum circuit.
...
def run_keep_basis_check(qvm, TargetProc,
    ...
    """
    Run keep-basis checking for a quantum circuit.
