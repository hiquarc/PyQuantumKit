...
- 参数`q_circuit`指定目标量子线路，它的类型是各量子开发框架的量子线路类（例如qiskit的`QuantumCircuit`、pyqpanda3的`QCircuit`或`QProg`、quafu的`QuantumCircuit`或cqlib的`Circuit`）或PyQuantumKit的`CircuitIO`类（见后文）。
