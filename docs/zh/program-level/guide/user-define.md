...
### 初始化操作`_initialize_`
在对量子变量执行`declare_qvars()`操作时，量子变量的初始化操作`_initialize_`会被自动调用，通常用于为量子变量制备初始量子态。初始化操作类似于C++中的构造函数或Python类的`__init__`方法。
...
以下程序`qmain1`声明一个初态为 $\ket{\Psi^+}$ （对应字符串`'psi+'`）的`BellQubitPair`变量，并命名为`'mybell'`：
```python
